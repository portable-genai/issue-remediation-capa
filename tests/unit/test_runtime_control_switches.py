...
this service has: ``CAPA_REVIEW_ROUTING`` is read in three states; off binds a disabled router and
says so at startup; on under the managed profile refuses to boot without a console; and the API
(triage and issue assessment), the agent tool and the CLI report ``review_routing`` rather than
...
from issue_remediation_capa.adapters.controls import (
    DisabledReviewRouter,
    RecordingReviewRouter,
    ReviewRouting,
)
from issue_remediation_capa.agent import tools
from issue_remediation_capa.api import app as api_module
from issue_remediation_capa.api.app import app
from issue_remediation_capa.cli.main import main as cli_main
...
from issue_remediation_capa.domain.models import TriageInput, TriageResult
from issue_remediation_capa.domain.triage_service import TriageService
from issue_remediation_capa.envread import ConfiguredEmptyError
...
_LOCAL_ROUTE = "issue_remediation_capa.adapters.local.review_router.LocalReviewRouter.route"
...
def _gcp(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "issue_remediation_capa.config.resolve_profile",
...
def test_the_off_posture_is_logged_once_however_many_containers(
    ...
    settings = Settings(profile="local", controls=ControlSwitches(review_routing=False))
    with caplog.at_level(logging.WARNING, logger="issue_remediation_capa.config"):
        for _ in range(3):
...
def test_a_failed_hand_off_is_reported_and_logged_never_raised(
    ...
    with caplog.at_level(logging.WARNING, logger="issue_remediation_capa.adapters.controls"):
...
# --------------------------------------------------------------------------- #
# The issue assessment hands off too
# --------------------------------------------------------------------------- #
_ASSESS = {
...
def test_the_assessment_reports_routed_off_and_failed(monkeypatch: pytest.MonkeyPatch) -> None:
    assert _post("/v1/issues/assess", _ASSESS)["review_routing"] == "routed"

    monkeypatch.setattr(_LOCAL_ROUTE, _Refusing.route)
    failed = _post("/v1/issues/assess", _ASSESS)
    assert failed["review_routing"] == "failed"
    assert failed["review_ref"] == ""

    monkeypatch.setenv(REVIEW_ROUTING_ENV, "off")
    api_module._container.cache_clear()
    off = _post("/v1/issues/assess", _ASSESS)
