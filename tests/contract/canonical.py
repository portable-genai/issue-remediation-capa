...
CANONICAL_GENERATION = GenerationRequest(
    system="restate the remediation figures",
    prompt="Issue: demo. Facts: overdue_business_days=2, missing_evidence=1.",
    facts=(("overdue_business_days", "2"), ("missing_evidence", "1")),
)
...
def _intake_invoke(adapter: Any) -> Any:
    return adapter.fetch(IssueSource.AUD1_FINDING)


def _intake_answered(_adapter: Any, result: Any) -> bool:
    return isinstance(result, tuple) and len(result) > 0


def _embeddings_invoke(adapter: Any) -> Any:
    return adapter.embed(("alpha residency control", "beta encryption key"))


def _embeddings_answered(_adapter: Any, result: Any) -> bool:
    return isinstance(result, tuple) and len(result) == 2 and all(len(vec) > 0 for vec in result)
...
CANONICAL_CALLS: dict[str, PortCase] = {
    ...
    "embeddings": PortCase(
        ...
        # The managed embedder lazily imports the Vertex SDK, absent offline and in CI.
        managed_refusal=(ImportError,),
        detail="embed issue text into vectors for clustering",
    ),
    "tracer": PortCase(
