# SPEC: Issue Remediation and CAPA Tracker (`issue-remediation-capa`)
...
- **Maker-checker (P-06) and routing (R8)**: a HIGH/CRITICAL result sets
  ...
  configured refuses at boot; `CAPA_REVIEW_ROUTING=off` is the stated way to run without it.
- **Profile**: resolved ONCE, at import, into a `ProfileChoice` and never a bare string. Three
  states of `CAPA_PROFILE`: UNSET is NO CHOICE (the SDK-free adapters
