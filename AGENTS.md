...
- `src/issue_remediation_capa/domain/` is PURE stdlib. `kernel.py` holds the vertical-neutral
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `CAPA_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `CAPA_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `CAPA_PROFILE` (mentioning it in a refusal message is wanted, reading
