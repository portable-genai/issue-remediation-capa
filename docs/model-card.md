...
| `gcp` | `adapters/gcp/generation.py` | `adapters/gcp/embeddings.py` | Generation is Gemini via `google.generativeai`, imported lazily inside the method, model id pinned in the adapter as `_MODEL`, currently `gemini-3.5-flash`, with `response_mime_type=application/json`, no `temperature` (narration is drafting, so it samples freely and the parameter is omitted) and a caller-supplied `max_output_tokens` (the `GenerationRequest` default is 512). Embedding is Vertex AI via `vertexai.language_models.TextEmbeddingModel`, also lazily imported, model id pinned as `_MODEL`, currently `text-embedding-005`, initialised at `settings.region`. |
