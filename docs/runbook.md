# Runbook: Issue Remediation and CAPA Tracker (`issue-remediation-capa`)

## Deploy (gcp)
1. `CAPA_PROFILE=gcp`, install `.[gcp]`, region `asia-southeast1`.
...
3. Ingress is fronted by IAP/Apigee; the app authenticates the S2S caller fail-closed
   (`CAPA_S2S_TOKEN` local, Google OIDC + allowlist secure).
4. **Set `CAPA_IAP_AUDIENCE`.** Without it the service starts, stays
...
`CAPA_IAP_AUDIENCE` is the IAP-protected resource the assertion must be
...
| Symptom | Cause | Fix |
|---|---|---|
| 503, detail names `CAPA_IAP_AUDIENCE` | no audience configured | set it to the protected resource above |
...
| `gcp` IAP assertion | `verified` | authenticated: the signature (against IAP's own key set), the audience (against `CAPA_IAP_AUDIENCE`), the expiry and the issuer are all checked before any claim is read |
...
1. **Nobody chose a profile.** `CAPA_PROFILE` is absent, so no end-user
...
2. **The `local` profile, chosen deliberately.** The seeded personas are a client-asserted
   ...
   `CAPA_S2S_TOKEN` is set. Setting that secret closes the S2S
...
- `add_loopback_exposure_guard` is registered at module scope in `api/app.py`, so it holds under
  `uvicorn issue_remediation_capa.api.app:app --host 0.0.0.0` (what the Dockerfile `CMD`
...
Set `CAPA_ALLOW_INSECURE_DEMO=1` to accept the exposure deliberately.
...
`CAPA_PROFILE` is read once, in `config.py`, and it has three states:
...
`CAPA_S2S_TOKEN`. With routing on and the URL unset, the managed profile REFUSES TO BOOT, so a
...
`CAPA_REVIEW_ROUTING` switches routing, read in three states: unset is on, `true`/`false` (or
`on`/`off`) wins, and an emptied or unrecognised value refuses at boot. Off needs no console, logs
one warning at startup, and every triage and issue assessment that requires review reports
...
`CAPA_AUDIT_ANCHOR` (read by `audit_anchor_path` in
`config/settings.yaml`) to a file on a DIFFERENT volume from
`CAPA_AUDIT_PATH`, ideally writable by a different principal. This is
not decoration:
