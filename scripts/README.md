...
| `load_demo_book.py` | Loads the shipped fictional issue landing rows (`src/issue_remediation_capa/data/demo_book/`) into the managed BigQuery dataset, so the deployment triages the same records the laptop reads from DuckDB rather than querying five tables nothing created. It fills tables and never creates them, so the Terraform must be applied first, and it refuses to overwrite a dataset whose own `book_manifest` does not declare what it holds fictional. The two deliberately broken records load with NULLs, because a warehouse row cannot omit a key: they must still be DROPPED rather than defaulted. `--dry-run DIR` writes the NDJSON it would load and stops, needing no credentials and no `[gcp]` extra. |
