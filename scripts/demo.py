...
from issue_remediation_capa.domain import (
    kernel,
    models,
)
...
SERVICE_NAME = "Issue Remediation and CAPA Tracker"
CATALOG_ID = "issue-remediation-capa"
REPOSITORY = "issue-remediation-capa"
...
def _exit_generation(container: Any) -> Any:
    return container.generation.generate(
        GenerationRequest(
            ...
            prompt="Facts: overdue_business_days=2, missing_evidence=1.",
            facts=(("overdue_business_days", "2"), ("missing_evidence", "1")),
        )
    )


def _exit_intake(container: Any) -> Any:
    return container.intake.fetch(IssueSource.AUD1_FINDING)


def _exit_embeddings(container: Any) -> Any:
    return container.embeddings.embed(("alpha residency control", "beta encryption key"))
