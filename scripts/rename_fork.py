...
The catalog id (``Aud3``) is left alone unless ``--catalog-id`` is given: it names the entry in
...
_OLD_PACKAGE = "issue_remediation_capa"
_OLD_ENV_PREFIX = "CAPA_"
_OLD_BARE_PREFIX = "CAPA"
_OLD_RESOURCE = "aud3-svc"
_OLD_DIST = "issue-remediation-capa"
_OLD_CATALOG_ID = "Aud3"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (CAPA_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource",
        required=True,
        help="new cloud resource stem (Terraform name_prefix), e.g. acme-capa",
    )
