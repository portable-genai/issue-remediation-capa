# ARCHITECTURE: Issue Remediation and CAPA Tracker (`issue-remediation-capa`)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`CAPA_PROFILE`) swaps the
...
## Layout (`src/issue_remediation_capa/`)
