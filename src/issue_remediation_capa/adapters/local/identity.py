...
``live``, which is ``local`` with a real local model behind the model port) AND (when the settings
came from the environment) ``CAPA_PROFILE`` must have been set rather
...
class LocalIdentityAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not settings.profile_explicit:
            raise LocalPersonaProfileError(
                "CAPA_PROFILE is not set, so the local profile was "
                "inherited rather than chosen; seeded dev personas grant their groups and "
                "tenant with no authentication and are refused. Set "
                "CAPA_PROFILE=local deliberately for a dev or demo "
                "run, or CAPA_PROFILE=gcp for a real deployment."
            )
