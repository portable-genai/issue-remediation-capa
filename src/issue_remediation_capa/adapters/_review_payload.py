...
def _kit_citations(result: TriageResult) -> tuple[KitCitation, ...]:
    ...
    A locator is routinely built from client text (``case:<the case subject>``) and a title from
    an intake feed's own wording, so masking only the snippet let the identifier cross to the
    shared console in the fields named like keys. De-duplication keys off the REDACTED locator,
    so two cases that differ only in a masked identifier collapse to one citation rather than
    both crossing the wire.
    """
    seen: set[str] = set()
...
def result_to_review(result: TriageResult, *, maker: str, tenant: str = "") -> Review:
    ...
    because ``pii_kit.redact`` substitutes a fixed literal token per pattern, with no hash and
    no salt, so the same subject always yields the same masked key.
    """
    subject = _redact(result.subject)
    return Review(
        action="issue_remediation_capa:triage",
        subject=subject,
        maker=maker,
        tenant=tenant,
        ...
        sod_group="issue_remediation_capa-maker-checker",
        case_ref=subject,
        # Producer-owned, tenant-scoped key so a retried delivery is idempotent at the console.
        source_key=f"issue-remediation-capa:{subject}:{result.severity.value}",
