...
_PROFILE_ENV = "CAPA_PROFILE"
_SETTINGS_ENV = "CAPA_SETTINGS"
...
class ProfileChoice:
    ...
    The variable is ``CAPA_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("CAPA_PROFILE", "local")``: that fallback reads an
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``CAPA_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "CAPA_REVIEW_ROUTING"
...
class Container:
    ...
    @cached_property
    def intake(self) -> IssueIntakePort:
        adapter = self._bind("intake")
        assert isinstance(adapter, IssueIntakePort)
        return adapter
