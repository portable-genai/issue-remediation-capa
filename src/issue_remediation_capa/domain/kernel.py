...
def redacted_citations(citations: tuple[Citation, ...]) -> tuple[Citation, ...]:
    ...
    case subject>``, an intake feed's ``finding_id``), so both are raw client text wearing a
...
class AuditEvent:
    ...
    redacted loses nothing by the second pass, and a caller that never redacted at all (the CAPA
    assessment path passed its engine-built summary straight through) is covered too.
