...
``CAPA_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``CAPA_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from ..domain.rca import RcaService
from ..domain.triage_service import TriageService
from ..pipeline import build_themes, collect_issues
...
_TOKEN_ENV = "CAPA_S2S_TOKEN"
_INSECURE_DEMO_ENV = "CAPA_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "CAPA_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "CAPA_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "CAPA_S2S_AUDIENCE"
_API_HOST_ENV = "CAPA_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``CAPA_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``CAPA_PROFILE`` must never be allowed to
    ...
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "CAPA_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "issue-remediation-capa"
...
_APP_TITLE = "Issue Remediation and CAPA Tracker"
...
#
# Note what is NOT in this expression: CAPA_S2S_TOKEN. A service
...
def _capa_to_review(assessment: CapaAssessment) -> TriageResult:
    """Bridge a CAPA assessment onto the shared R8 review envelope, at the surface.

    The review path is shared across the fleet and speaks the kernel result shape, so a
    consequential lifecycle assessment is projected onto it here rather than the pure domain
    importing a surface type. The fields carry across unchanged.
    """
    return TriageResult(
...
def _one_issue(source: IssueSource, external_id: str, *, container: Container) -> IssueEnvelope:
    """Fetch and normalize exactly the issue named by ``source`` + ``external_id`` (or 404)."""
    for envelope in collect_issues(container.intake, source):
        if envelope.external_id == external_id:
            return envelope
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail=f"no issue {external_id!r} from source {source.value!r}",
    )


@app.post("/v1/issues/assess", response_model=IssueAssessResponse, tags=["artifacts"])
def assess_issue(
    ...
    """
    try:
        authorize_issue_access(principal.tenant)
    except CrossTenantError as exc:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc)) from exc

    try:
        source = IssueSource(request.source)
        state = LifecycleState(request.state)
        as_of = date.fromisoformat(request.as_of)
        state_since = date.fromisoformat(request.state_since)
    ...
    container = _container()
    envelope = _one_issue(source, request.external_id, container=container)
    record = IssueRecord(
        envelope=envelope,
        state=state,
        state_since=state_since,
        provided_evidence=tuple(request.provided_evidence),
...
def themes(principal: Annotated[Principal, Depends(get_principal)]) -> ThemesResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc)) from exc
    container = _container()
    built = build_themes(container.intake, container.embeddings)
    return ThemesResponse(themes=[ThemeModel.from_domain(theme) for theme in built])
